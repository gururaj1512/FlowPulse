          ...
          {runs.map(run => {
            ...
            return (
              ...
              >
                ...
                {/* Floating Hover Card Detail Popover */}
                {isHovered && (
                  <td colSpan="6" style={{ padding: 0, border: 'none' }}>
                    <div className="execution-popover-card">
                      <div className="flex items-center justify-between mb-sm pb-xs" style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                        <div className="flex items-center gap-sm">
                          <span className={`status-badge status-${run.status}`}>{run.status}</span>
                          <span className="text-xs font-mono text-gold font-bold">ID: {run.id}</span>
                        </div>
                      ...
                      <div className="text-xs font-mono text-secondary mb-sm">
                        STEP RUN DETAILS:
                      </div>
                      ...
                      <div className="flex flex-col gap-xs mb-md">
                        {run.step_runs?.map((sr, i) => (
                          <div 
                            ...
                            style={{ background: 'var(--bg-primary)', borderRadius: '4px', padding: '6px 10px' }}
                          >
                            <div className="flex items-center gap-sm">
                              ...
                              <span className={`step-type-tag step-${sr.workflow_step?.type}`} style={{ fontSize: '0.65rem', padding: '1px 6px' }}>
                                {sr.workflow_step?.type?.replace('_', ' ')}
                              </span>
                            </div>

                            <div className="flex items-center gap-sm">
                              <span className={`status-badge status-${sr.status}`} style={{ fontSize: '0.65rem', padding: '2px 8px' }}>
                                {sr.status}
                              </span>
                              {sr.attempt_count > 1 && (
                                <span className="text-xs text-gold">({sr.attempt_count} attempts)</span>
                              )}
                            </div>
                          </div>
                        ))}
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-xs text-muted font-mono">
                          ⚡ Real-time GraphQL Subscription Enabled
                        </span>
                        <span className="btn btn-primary btn-sm" style={{ padding: '4px 12px', fontSize: '0.7rem' }}>
                          CLICK TO OPEN LIVE RUN VIEWER ↗
                        </span>
                      </div>
                    </div>
                  </td>
